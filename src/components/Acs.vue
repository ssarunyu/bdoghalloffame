<script setup>
  ...
  async function testFindLastMatch() {
    if(store.getAcsResult().length === 0) {
      ...
      console.log(store.getAcsResult())
    }
  }
  ...
  const sorted = computed(() => {
    // Most to less
    return [...store.getAcsResult()].sort((a, b) => b.lacs - a.lacs)
  })
      ...
      <div class="w-full max-w-4xl px-4 sm:px-0">
        ...
        <div class="mt-5 space-y-5">
          <div class="p-3 bg-slate-700 text-white flex justify-between items-center rounded-md shadow-sm" v-for="(player, index) in sorted">
            ...
            <div class="w-24 text-center">{{ player.lacs }}</div>
          </div>
        </div>
      </div>
    </div>
    <div class="flex justify-center mt-10">
      <button v-show="!isLoading" class="px-6 py-2 bg-white text-black rounded-md shadow hover:bg-slate-300 transition"
      @click="testFindLastMatch()">
      Refresh
    </button>
    </div>
  </div>
